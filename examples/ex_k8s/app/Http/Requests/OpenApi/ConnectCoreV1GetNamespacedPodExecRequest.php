<?php

// Code generated by lcodegen, DO NOT EDIT.

namespace App\Http\Requests\OpenApi;

use App\Http\Dto\OpenApi as Dto;
use Illuminate\Foundation\Http\FormRequest;

/**
 * Auto-generated form request from OpenAPI specification.
 */
abstract class ConnectCoreV1GetNamespacedPodExecRequest extends FormRequest
{
    /**
     * Get the validation rules that apply to the request.
     *
     * @return array<string, \Illuminate\Contracts\Validation\ValidationRule|array<mixed>|string>
     */
    public function rules(): array
    {
        return [
            'command' => ['sometimes', 'string'],
            'container' => ['sometimes', 'string'],
            'stderr' => ['sometimes', 'boolean'],
            'stdin' => ['sometimes', 'boolean'],
            'stdout' => ['sometimes', 'boolean'],
            'tty' => ['sometimes', 'boolean'],
        ];
    }

    /**
     * Get the validated data from the request.
     *
     * @param array|int|string|null $key
     * @param mixed $default
     * @return Dto\ConnectCoreV1GetNamespacedPodExecParams|mixed
     */
    public function validated($key = null, $default = null): mixed
    {
        $validated = parent::validated($key, $default);
        if ($key === null) {
            return Dto\ConnectCoreV1GetNamespacedPodExecParams::fromArray($validated);
        }

        return $validated;
    }
}
